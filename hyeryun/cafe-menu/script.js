...
var menu = document.querySelector('#cafe-menu'); // id가 cafe-menu인 div node(element)를 menu 변수로 저장
...
function renderItems(list){
    list.forEach(function(item){
        // item에는 list의 각 요소가 저잔된다.

        // getCategory 함수에서 item의 현재 category에 해당하는 html이
        // 있으면 그 html tag(element)를 가져오고
        // 없으면 templates에서 cstegory 요소를 복사한 뒤에, 그 요소를 menu에 넣고, 해당 요소를 가져온다.
...
function getCategory(item){
    ...
    if( !category ){ // null이란 데이터는 아무것도 없는 변수라는 뜻 !(null) === false
        ...
        // deep == true인 경우, 해당 tag의 자식 tag들도 함께 복사
        ...
        category.querySelector('h3').innerText = item.category; // category html의 h3 태그에 item의 category 테이터를 text로 입력
